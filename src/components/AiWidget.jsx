    ...
    <span style={{ display: "inline-flex", gap: 4, alignItems: "center" }}>
      {[0, 1, 2].map(i => (
        <span key={i} style={{
          ...
          background: "#00f5ff",
          animation: `wPulse 1.2s ${i * 0.2}s ease-in-out infinite`,
...
export default function AiWidget() {
  ...
  const [messages, setMessages] = useState([{
    sender: "bot",
    text: "SYSTEM ONLINE // Xin chào! Tôi là AI Assistant. Hỏi tôi bất cứ điều gì về dữ liệu hệ thống.",
  }]);
  ...
  // Pulse the button every 5s when closed
  useEffect(() => {
    if (isOpen) return;
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { sender: "bot", text: "⚠ CONNECTION FAILED — Vui lòng thử lại sau." }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <button
              ...
              style={{
                ...
                background: "linear-gradient(135deg,#001a2e,#003d60)",
                color: "#00f5ff", border: "2px solid #00f5ff44",
                boxShadow: "0 0 20px rgba(0,245,255,0.3), inset 0 0 20px rgba(0,245,255,0.08)",
                cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 26, transition: "all 0.2s",
                animation: pulse ? "wPulseScale 0.7s ease-in-out" : "wGlow 2s ease-in-out infinite",
              }}
              onMouseEnter={e => { e.currentTarget.style.transform = "scale(1.12)"; e.currentTarget.style.boxShadow = "0 0 35px rgba(0,245,255,0.5), inset 0 0 20px rgba(0,245,255,0.15)"; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.boxShadow = "0 0 20px rgba(0,245,255,0.3), inset 0 0 20px rgba(0,245,255,0.08)"; }}
            >
              🤖
            </button>
            {/* Notification badge */}
        ...
        {isOpen && (
          <div style={{
            ...
            boxShadow: "0 0 40px rgba(0,245,255,0.15), 0 20px 60px rgba(0,0,0,0.8), inset 0 0 40px rgba(0,245,255,0.03)",
            display: "flex", flexDirection: "column",
            animation: "wMsgIn 0.25s ease-out",
            position: "relative",
          }}>
            {/* Grid background */}
            ...
            <div style={{ padding: "14px 18px", background: "rgba(0,12,28,0.95)", borderBottom: "1px solid rgba(0,245,255,0.15)", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 2, flexShrink: 0 }}>
              {/* Scanline in header */}
              <div style={{ backgroundImage: "repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,245,255,0.02) 2px,rgba(0,245,255,0.02) 4px)", position: "absolute", inset: 0, pointerEvents: "none" }} />
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ color: "rgba(0,245,255,0.45)", fontSize: 10, fontFamily: "monospace", letterSpacing: "0.5px" }}>
                    {loading ? "▸ PROCESSING QUERY..." : "▸ ONLINE · READY"}
                  </div>
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <button onClick={() => setIsOpen(false)} style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", color: "#f87171", borderRadius: 6, width: 26, height: 26, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 900, transition: "all 0.15s" }}
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10, position: "relative", zIndex: 2 }}>
              {messages.map((msg, i) => (
                <div key={i} className="w-msg" style={{ display: "flex", justifyContent: msg.sender === "user" ? "flex-end" : "flex-start", gap: 8, alignItems: "flex-end" }}>
                  {msg.sender === "bot" && (
                    <div style={{ width: 24, height: 24, borderRadius: "50%", background: "linear-gradient(135deg,#001a2e,#003d60)", border: "1px solid rgba(0,245,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0, boxShadow: "0 0 8px rgba(0,245,255,0.2)" }}>🤖</div>
                  )}
                  <div style={{
                    ...
                    background: msg.sender === "user" ? "linear-gradient(135deg,rgba(99,102,241,0.25),rgba(59,130,246,0.15))" : "rgba(0,20,45,0.85)",
                    border: `1px solid ${msg.sender === "user" ? "rgba(99,102,241,0.35)" : "rgba(0,245,255,0.15)"}`,
                    color: msg.sender === "user" ? "#c7d2fe" : "#9de8f0",
                    fontFamily: msg.sender === "bot" ? "'Courier New', monospace" : "inherit",
                    boxShadow: msg.sender === "user" ? "0 0 12px rgba(99,102,241,0.15)" : "0 0 12px rgba(0,245,255,0.08)",
                    fontSize: 12,
                  }}>{msg.text}</div>
                  {msg.sender === "user" && (
            ...
            <div style={{ padding: "10px 14px 14px", borderTop: "1px solid rgba(0,245,255,0.1)", background: "rgba(0,8,20,0.98)", position: "relative", zIndex: 2, flexShrink: 0 }}>
              ...
              <form onSubmit={handleSend} style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1, position: "relative" }}>
                  {focused && (
                    <div style={{ position: "absolute", inset: -1, borderRadius: 11, background: "linear-gradient(90deg,#00f5ff,#6366f1,#00f5ff)", backgroundSize: "200%", animation: "wHue 3s linear infinite", opacity: 0.5, zIndex: 0 }} />
                  )}
                  <input
                    className="w-input"
                    style={{ position: "relative", zIndex: 1, width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid rgba(0,245,255,0.2)", background: "rgba(0,10,30,0.95)", color: "#00f5ff", fontSize: 12, fontFamily: "monospace", outline: "none", boxSizing: "border-box", boxShadow: focused ? "0 0 14px rgba(0,245,255,0.12)" : "none", transition: "box-shadow 0.2s" }}
                ...
                <button
                  ...
                  style={{
                    ...
                    color: loading || !input.trim() ? "rgba(0,245,255,0.25)" : "#00f5ff",
                    cursor: loading || !input.trim() ? "not-allowed" : "pointer",
                    fontFamily: "monospace", fontWeight: 900, fontSize: 12, letterSpacing: "1px",
                    boxShadow: !loading && input.trim() ? "0 0 16px rgba(0,245,255,0.2)" : "none",
